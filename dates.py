...
def voeg_adressen_en_kantoordag_toe(df_met_uren):
    ...
    for index, row in df_met_uren.iterrows():
        ...
        if dag_naam == 'vrijdag':
            if rbi_friday == 'ja':
                is_kantoordag = False # RBI Friday is geen kantoordag (volgens instructie)
...
def controleer_en_vul_contracturen(df_final):
    ...
    for (jaar, week), group in df_werk.groupby(['iso_jaar', 'iso_week']):
        ...
        if totaal_uren < contract_uren:
            ...
            keuze = input(f"   Wil je {tekort} uur toevoegen aan de vrijdag van deze week? (j/n): ").strip().lower()
            ...
            if keuze == 'j':
                ...
                if not vrijdag_rijen.empty:
                    ...
                    if 'iso_jaar' in bron_rij: del bron_rij['iso_jaar']
                    if 'iso_week' in bron_rij: del bron_rij['iso_week']
                    
                    nieuwe_rijen.append(bron_rij)
                    print(f"   ✅ {tekort} uur toegevoegd aan vrijdag {bron_rij['datum'].strftime('%Y-%m-%d')}.")
                else:
                    print("   ❌ Geen vrijdag gevonden in de selectie van deze week. Kan niet kopiëren.")
            else:
